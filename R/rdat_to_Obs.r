...
rdat_to_Obs <- function(
  ...
  if(min(a.series$age)>0){
    message(paste0(Name,": Minimum age > 0. Age-based data extrapolated to age-0"))
  ...
  if(!Ind_abb[1]=="none"){
    IndCalc <- local({
      ...
      if(length(Ind_names)==0){
        message(paste0(Name,": Ind_abb does not match any index names in the rdat t.series. Ind will be the geometric mean of all available indices: ",paste(Ind_names_all,collapse=", ")))
        Ind_names <- Ind_names_all
      }
  ...
  if(!CAA_abb[1]=="none"){
    ...
    if(any(is_acomp)){
      ...
      if(length(acomp_names)==0){
        message(paste0(Name,": CAA_abb does not match any names in the rdat comp.mats. CAA will be computed as a cellwise average of all available age compositions: ",paste(acomp_names_all,collapse=", ")))
        acomp_names <- acomp_names_all
        acomp_n_names <- acomp_n_names_all
        acomp_nfish_names <- acomp_nfish_names_all
        acomp_neff_names <- acomp_neff_names_all
      ...
      message(paste0(Name,": No acomp found in comp.mats."))
    }
  }
  ...
  if(!CAL_abb[1]=="none"){
    ...
    if(any(is_lcomp)){
      ...
      if(length(lcomp_names)==0){
        message(paste0(Name,": CAL_abb does not match any names in the rdat comp.mats. CAL will be computed as a cellwise average of all available length compositions: ",paste(lcomp_names_all,collapse=", ")))
        lcomp_names <- lcomp_names_all
        lcomp_n_names <- lcomp_n_names_all
        lcomp_nfish_names <- lcomp_nfish_names_all
        lcomp_neff_names <- lcomp_neff_names_all
    ...
    lcomp_data_nfisheff <- local({
      ...
      message(paste0(Name,": No lcomp found in comp.mats."))
    }
  }
  ...
  if(!any(dim(acomp_data_neff)==0)){
    ...
    message(paste0(Name,": Cannot estimate CAA_ESS from empirical data. Scaling CAA_nsamp by CAA_ESS_sc to compute CAA_ESS."))
  ...
  if(!any(dim(lcomp_data_neff)==0)){
    ...
    message(paste0(Name,": Cannot estimate CAL_ESS from empirical data. Scaling CAL_nsamp by CAL_ESS_sc to compute CAL_ESS."))
