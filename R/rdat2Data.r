...
if(combine_indices){
  ...
  if(identical(AddInd_order,"CVlo")){
    CV_AddInd_o <- order(apply(CV_AddInd,2,function(x){mean(x,na.rm=TRUE)}))
    AddInd_order <- AddInd_names[CV_AddInd_o]
  }
